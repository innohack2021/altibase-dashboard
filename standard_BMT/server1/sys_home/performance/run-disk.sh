...
case $ARG_NAME in
    TPC-C)
       ...
       echo $PWD
       sh run-disk.sh -p $ARG_PRODUCT -t $ARG_TIME -i $ARG_INTERFACE -g $ARG_SCALE -k $ARG_CONNECTION_COUNT
        ;;
    TPC-H)
       cd $PERF_SOURCE/$ARG_NAME
       sh run-disk.sh -p $ARG_PRODUCT -s $ARG_SCALE
        ;;
    COMVERSE)
        ;;
    STANDARDBMT)
       cd $PERF_SOURCE/$ARG_NAME
       echo $PWD
       sh run.sh -p $ARG_PRODUCT -i $ARG_INTERFACE -g $ARG_SCALE -s disk
        ;;
    DBLINK)
       cd $PERF_SOURCE/$ARG_NAME
       echo $PWD
