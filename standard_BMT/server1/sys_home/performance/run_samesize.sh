...
while getopts "p:s:n:d:t:c:i:h:r:k:" opt
do
    case $opt in
	p) ARG_PRODUCT=`echo $OPTARG | tr '[a-z]' '[A-Z]'`
        ...
        k) ARG_CONNECTION_COUNT=`echo $OPTARG | tr '[a-z]' '[A-Z]'`
                ...
                ;;
        r) ARG_ROW_COUNT=`echo $OPTARG | tr '[a-z]' '[A-Z]'`
                echo "Test row count is $ARG_ROW_COUNT"
                ;;
...
case $ARG_NAME in
    TPC-C)
       ...
       echo $PWD
       sh run.sh -p $ARG_PRODUCT -t $ARG_TIME -i $ARG_INTERFACE -g $ARG_SCALE -k $ARG_CONNECTION_COUNT
        ;;
    TPC-H)
       cd $PERF_SOURCE/$ARG_NAME
    ...
    STANDARDBMT)
       ...
       if [ $ARG_SCALE ]
       then
                sh run_samesize.sh -p $ARG_PRODUCT -i $ARG_INTERFACE -g $ARG_SCALE -s memory
        else
                sh run_samesize.sh -p $ARG_PRODUCT -i $ARG_INTERFACE -r $ARG_ROW_COUNT -s memory
        fi
        ;;
    DBLINK)
       cd $PERF_SOURCE/$ARG_NAME
    ...
    SIMPLEBMT)
       ...
       echo $TESTNAME 
       sh run.sh -p $ARG_PRODUCT -i $ARG_INTERFACE -g $ARG_SCALE -s memory
        ;;
    LAZYREPLICATION)
        ;;
