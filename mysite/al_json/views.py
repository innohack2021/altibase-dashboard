...
def to_json():
    ...
    try :
        ...
        for scale in db_json:
            for version in db_json[scale]:
                for thread in db_json[scale][version]:
                    if thread != 'info':
                        for data_control in db_json[scale][version][thread]:
                            for test_result in db_json[scale][version][thread][data_control]:
                                if 'TEST_RESULT' in test_result:
                                    if (float(db_json[scale][version][thread][data_control][test_result]) > 100000):
                                        db_json[scale][version][thread][data_control][test_result] = '0'
